...
## 사용자 수정 지시 — 상단에 실제 영상 배치
사용자는 작은 로고 표시가 아니라 채팅에서 본 애니메이션 자체를 사이트 상단에 배치하도록 명확히 지시했다. 승인된 5초 MP4를 첫 화면의 큰 영상 영역에 직접 넣고, 무음·인라인·반복 재생한다. 기존 상태 판정과 후원 영역은 유지한다. 운영 중에만 자동 재생하며 유지보수·지연·미확인 때에는 영상을 멈추고 어두운 정지 이미지를 표시한다. 화면 숨김·동작 줄이기를 존중하고 재생 차단 시 직접 재생 버튼을 제공한다. 공통 발행 목록에 MP4를 포함하고 실제 브라우저의 currentTime 증가와 정지·복구를 검증한다. 배포 cooldown은 계속 준수한다.
