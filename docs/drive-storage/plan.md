...
- 최대 4개의 업로드 worker와 8개의 미완료 chunk로 메모리/요청 수를 제한한다.
- 동일 SHA chunk는 미완료 요청을 공유한다. 재사용 객체의 무결성 비교도 유지한다.
- worker 실패를 호출자에게 전달하고 모든 업로드 성공 이후에만 manifest를 확정한다.
- OAuth refresh는 lock으로 직렬화한다. 기존 저장 형식과 원본 파일은 바꾸지 않는다.
- 겹치는 동일 chunk, worker 오류, 복원 결과를 확인한 후 CI를 통과시켜 반영한다.
