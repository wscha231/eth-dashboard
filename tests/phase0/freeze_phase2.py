"""Re-freeze walk-forward metrics including the Phase 2 skill-weighted ensemble.

Builds on Phase 1.3: same fold-safe feature selection, same purged+embargo CV,
but now appends both an equal-weight ensemble (for comparison) and the
skill-weighted ensemble (Phase 2 default) to each leaderboard. Output lands in
``phase2_metrics.{pkl,json}`` — Phase 1 pickles are never overwritten.
"""
...
OUTPUT_PKL = Path(__file__).with_name("phase2_metrics.pkl")
OUTPUT_JSON = Path(__file__).with_name("phase2_metrics.json")
...
def run_inline(master_data_csv: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "mode": "phase2_skill_weighted_ensemble",
        "cv_test_size": cv_test_size,
