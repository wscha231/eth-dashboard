"""Phase 4B production freeze: Phase 4 synergy-aware member selection plus
drop Phase 3B macro composites. The Phase 3B ablation showed those composites
regress h=30 classification calibration at the individual-model level (best
single brier 0.208 -> 0.224), not only via member selection; synergy
diversification alone cannot recover that. Phase 3 Deribit features remain
dormant in the candidate list (blocked by min_feature_coverage=0.03 anyway).

Output: phase4b_production_metrics.{pkl,json}.
"""
...
OUTPUT_PKL = Path(__file__).with_name("phase4b_production_metrics.pkl")
OUTPUT_JSON = Path(__file__).with_name("phase4b_production_metrics.json")
...
def run_inline(master_data_csv: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "mode": "phase4b_production_phase4_selection_minus_phase3b",
        "cv_test_size": cv_test_size,
