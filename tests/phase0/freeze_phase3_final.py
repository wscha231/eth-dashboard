"""Re-freeze walk-forward metrics with Phase 3 (Deribit-regime) + Phase 3B
(macro-regime composites) features. Phase 3A is a collector-side change;
its effect only lands once fresh vendor data is pulled, so the CV window
here mostly exercises Phase 3B additions plus the already-applied Phase 3
dormant Deribit features.

Output: phase3_final_metrics.{pkl,json}.
"""
...
OUTPUT_PKL = Path(__file__).with_name("phase3_final_metrics.pkl")
OUTPUT_JSON = Path(__file__).with_name("phase3_final_metrics.json")
...
def run_inline(master_data_csv: Path) -> dict[str, Any]:
    ...
    for horizon in horizons:
        ...
        phase3_present = [c for c in PHASE3_FEATURES if c in raw_candidates]
        phase3b_present = [c for c in PHASE3B_FEATURES if c in raw_candidates]
        full_mask = (
        ...
        full_dataset = feature_frame.loc[
            full_mask, raw_candidates + ["eth_close", "target_return", "target_close", *state_cols],
        ].copy()
        ...
        training_dataset = feature_frame.loc[
            training_mask, raw_candidates + ["eth_close", "target_return", "target_close", *state_cols],
        ].copy()
        ...
        reg_lb, reg_oof = efp.walk_forward_leaderboard(
            dataset=training_dataset, feature_columns=raw_candidates,
        ...
        per_horizon[horizon] = {
            ...
            "phase3_features_in_candidates": phase3_present,
            "phase3b_features_in_candidates": phase3b_present,
    ...
    return {
        ...
        "mode": "phase3_final_deribit_dormant_plus_macro_composites",
        "cv_test_size": cv_test_size,
