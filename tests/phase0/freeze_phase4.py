"""Phase 4 freeze: same feature set as Phase 3 Final (Deribit dormant + macro
composites) but the ensemble member selection is now synergy-aware, using
pairwise OOF-prediction correlation to break ties between high-skill peers.

Goal: recover h=30 classification calibration that regressed under Phase 3
Final while the new feature set shifted fold rankings toward near-duplicate
members.

Output: phase4_metrics.{pkl,json}.
"""
...
OUTPUT_PKL = Path(__file__).with_name("phase4_metrics.pkl")
OUTPUT_JSON = Path(__file__).with_name("phase4_metrics.json")
...
def run_inline(master_data_csv: Path) -> dict[str, Any]:
    ...
    for horizon in horizons:
        ...
        # Phase 4 change: pass OOF into the selectors so they can penalise
        # near-duplicate members via pairwise correlation.
    ...
    return {
        ...
        "mode": "phase4_synergy_aware_member_selection",
        "cv_test_size": cv_test_size,
