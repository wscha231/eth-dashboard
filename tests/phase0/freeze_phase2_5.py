"""Re-freeze walk-forward metrics under Phase 2.5 ensemble-selection rules.

Same CV setup as Phase 2 (purged+embargo, skill-weighted ensemble) but runs
against the tightened member-selection thresholds and best-single fallback in
``eth_price_forecast``. Output lands in ``phase2_5_metrics.{pkl,json}`` so that
Phase 2 pickles are preserved for diffing.
"""
...
OUTPUT_PKL = Path(__file__).with_name("phase2_5_metrics.pkl")
OUTPUT_JSON = Path(__file__).with_name("phase2_5_metrics.json")
...
def run_inline(master_data_csv: Path) -> dict[str, Any]:
    ...
    return {
        "frozen_at": datetime.now(tz=__import__("datetime").timezone.utc).isoformat(),
        "master_data_csv": str(master_data_csv),
        "eth_price_forecast_bytes": Path(efp.__file__).stat().st_size,
        "mode": "phase2_5_tight_member_selection_best_single_fallback",
        "cv_test_size": cv_test_size,
