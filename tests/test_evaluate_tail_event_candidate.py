...
from scripts.evaluate_tail_event_candidate import (
    BASE_MODELS,
    CATBOOST_MODEL,
    CLIMATOLOGY_MODEL,
    HEURISTIC_MODEL,
...
def test_full_outer_folds_expand_by_calendar_year_without_target_overlap() -> None:
    ...
    assert [fold.fold_id for fold in folds] == [str(year) for year in range(2019, 2027)]
    for fold in folds:
...
def test_frozen_pr1_evidence_is_rejected_and_cannot_enable_daily_integration() -> None:
    evidence_path = (
        Path(__file__).resolve().parent
        / "phase0"
        / "tail_event_pr1_rejected_metrics.json"
    )
    evidence = json.loads(evidence_path.read_text(encoding="utf-8"))

    assert evidence["evidence_status"] == "REJECTED"
    assert evidence["winner"]["primary_gate_checks"]["positive_brier_skill"] is False
    assert (
        evidence["winner"]["primary_gate_checks"]["episode_recall_at_least_35pct"]
        is False
    )
    assert "do not integrate" in evidence["decision"].lower()
