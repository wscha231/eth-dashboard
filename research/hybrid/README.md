...
7일과 30일의 직접 로그수익률을 각 발행시점의 과거 30일 변동성 × sqrt(horizon)으로 나누어 학습한다. 미래 가격 수준을 입력하지 않는다. 기준가격과 예측 수익률을 곱해 가격 차트를 만든다. v2는 각 월·기간·학습창의 이미 만기된 outer 학습 로그수익률 5/95 분위로 점예측 외삽을 제한한다. 0은 항상 제한 범위 안에 둔다. 원래 신경망 출력은 base_predictions.csv.gz에 보존하고 제한 횟수는 표와 다운로드에 공개한다. 이 점예측 제한은 불확실성 범위의 상하한과 별개다.

v1 전체 개발 재현에서 2020년 3월 급락 직후 변동성 확대가 Transformer 출력을 증폭하여 최대 +1,145.5%의 30일 점예측을 만들었다. v1의 30일 최적화 MAE는 24.665%p로 무변화 19.412%p보다 나빴다. 이 실패를 확인한 뒤 추가한 v2 제한은 공개된 개발 수정이다. 개선된 재현 점수를 새롭고 독립적인 시험 성적으로 주장하지 않는다. v1 결과와 실행 기록은 PR #20 및 data/hybrid-forecast 이력에 남긴다.
...
- 캐시 키는 실제 입력 OHLCV·지연된 원본 flow와 코드/버전으로 만든다. CPU별 log/correlation 계산의 미세한 차이만으로 동일 원자료를 재학습하지 않는다. 실제 과거 가격·flow 수정은 캐시를 무효화한다.
