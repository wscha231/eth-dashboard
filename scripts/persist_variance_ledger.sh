...
# other event ledgers, without mixing its schema into public/event issuance tables.
...
for name in ('active.json', 'report.json'):
    path = source / name
    if path.exists():
...
if ! git -C "$RUNNER_TEMP/variance-ledger" diff --cached --quiet; then
  git -C "$RUNNER_TEMP/variance-ledger" commit -m "chore(audit): preserve HAR-RV variance shadow"
